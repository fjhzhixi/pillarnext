    ...
    for split in ['train', 'val']:
        ...
        for idx, data_info in enumerate(track_iter_progress(data_infos)):
            ...
            for idx_box in range(len(gt_bboxes)):
                ...
                if ts[0] < pc_range[0] or ts[0] > pc_range[3] or ts[1] < pc_range[1] or ts[1] > pc_range[4] or ts[2] < pc_range[2] or ts[2] > pc_range[5]:
                    continue
                q_boxes.append(item.tensor[0])
            if len(q_boxes) == 0:
                sum_points = 0
            else:
                q_boxes = torch.stack(q_boxes)
                q_boxes = LiDARInstance3DBoxes(q_boxes)

                point_in_boxes = q_boxes.points_in_boxes(torch.tensor(points).to('cuda'))
                sum_points = sum(point_in_boxes!=-1).item()

            id = data_path.split('/')[-1].split('.')[0]
            results[id] = sum_points
            # break

    with open('tools/data_tools/point_in_boxes_filtered.json', 'w') as f:
